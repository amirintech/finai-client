...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant='ghost'
      size='icon'
      className={cn('my-4 size-7', className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
      {state == 'collapsed' ? <ArrowRightFromLine /> : <ArrowLeftFromLine />}
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium outline-hidden duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
